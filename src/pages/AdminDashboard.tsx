...
import { Clock, Users, Timer, Filter } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const url = queryString ? `admin_dashboard?${queryString}` : "admin_dashboard";
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-2xl font-bold">{data?.punches?.length ?? "—"}</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        {/* Hours per employee */}
        {data?.total_hours_per_employee && data.total_hours_per_employee.length > 0 && (
          <Card>
            <CardHeader>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {data.total_hours_per_employee.map((row) => (
